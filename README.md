...
[Acessar o repositório no GitHub](https://github.com/Arthurqzn/CP_Kotilin)
...
- Data e horário opcionais por tarefa, escolhidos com `DatePicker` e `TimePicker` do Material 3.
- Ordenação da lista: tarefas com prazo primeiro (da mais próxima para a mais distante) e, depois, as tarefas sem prazo (mais recentes primeiro).
- Destaque em vermelho e negrito para tarefas com prazo vencido que ainda não foram concluídas.
...
### 1. [`Tarefa.kt`](app/src/main/java/arthurqzn/com/github/todolist/data/Tarefa.kt) — modelo de dados e entidade do Room
...
| `dataCriacao` | Guarda o instante de criação e serve de critério de desempate na ordenação. |
| `dataHora` | Prazo opcional da tarefa, em milissegundos. `null` significa tarefa sem data e horário definidos. |
...
### 2. [`TarefaDAO.kt`](app/src/main/java/arthurqzn/com/github/todolist/data/TarefaDAO.kt) — comandos de acesso ao banco
...
- `@Query` contém o SQL da listagem. `ORDER BY dataHora IS NULL, dataHora ASC, dataCriacao DESC` aplica três critérios: primeiro as tarefas que têm prazo (`IS NULL` vale `0` para elas e `1` para as demais), depois o prazo mais próximo e, por fim, a tarefa criada mais recentemente.
...
### 3. [`TarefaDatabase.kt`](app/src/main/java/arthurqzn/com/github/todolist/data/TarefaDatabase.kt) — criação e compartilhamento do banco
...
`@Database` registra `Tarefa` como entidade e define a versão do esquema. A versão foi para `2` quando a coluna `dataHora` foi adicionada; `fallbackToDestructiveMigration(dropAllTables = true)` recria o banco nessa mudança, o que apaga tarefas gravadas por versões antigas do app (aceitável em um projeto acadêmico; em produção seria necessária uma `Migration`). A função abstrata `tarefaDao()` dá acesso ao DAO que o Room gera.
...
### 4. [`TarefaRepository.kt`](app/src/main/java/arthurqzn/com/github/todolist/repository/TarefaRepository.kt) — separação entre dados e apresentação
...
### 5. [`TarefaViewModel.kt`](app/src/main/java/arthurqzn/com/github/todolist/viewmodel/TarefaViewModel.kt) — estado da interface e operações
...
### 6. [`ListaTarefasScreen.kt`](app/src/main/java/arthurqzn/com/github/todolist/ui/ListaTarefasScreen.kt) — observação do estado e ações da lista
...
- quando existe `dataHora`, o card mostra o prazo formatado (`dd/MM/yyyy 'às' HH:mm`); se o prazo já passou e a tarefa não está concluída, o texto fica em vermelho e em negrito.

Se a lista estiver vazia, o código mostra `Nenhuma tarefa cadastrada.`. Os seis previews cobrem lista preenchida, lista vazia, item pendente, item concluído, item com prazo futuro e item atrasado sem precisar executar o app inteiro.

### 7. [`FormularioTarefaScreen.kt`](app/src/main/java/arthurqzn/com/github/todolist/ui/FormularioTarefaScreen.kt) — cadastro e edição na mesma tela
...
No modo de edição, `tituloInicial`, `descricaoInicial` e `dataHoraInicial` recebem os dados do objeto localizado. No modo de cadastro, título e descrição começam vazios e `dataHoraInicial` é `null`. Dentro de `FormularioTarefaContent`, `remember` e `mutableStateOf` guardam o texto digitado:
...
No cadastro, um novo objeto é criado com ID automático, `concluida = false` e data atual. Na edição, `copy()` preserva ID, conclusão e data de criação, substituindo somente título, descrição e prazo. Depois da operação, `onVoltar()` retorna à lista.
...
- impede título vazio e prazo incompleto por meio de `enabled = titulo.isNotBlank() && (!temDataHora || (ano != null && hora != null))`;
- exibe um `Switch` "Definir data e horário"; quando ligado, mostra dois botões que abrem o `DatePickerDialog` e o `TimePicker` (24 h). Ao desligar o `Switch` e salvar, a tarefa fica sem prazo (`dataHora = null`);
- combina ano, mês, dia, hora e minuto em um único timestamp com `combinarDataHora()`;
- usa uma seta de retorno com descrição de acessibilidade `Voltar`;
- possui previews separados para cadastro e edição.

### 8. [`AppNavigation.kt`](app/src/main/java/arthurqzn/com/github/todolist/navigation/AppNavigation.kt) — rotas e passagem do ID
...
### 9. [`MainActivity.kt`](app/src/main/java/arthurqzn/com/github/todolist/MainActivity.kt) — ponto de entrada da aplicação
...
### 10. [`DataHoraUtil.kt`](app/src/main/java/arthurqzn/com/github/todolist/util/DataHoraUtil.kt) — funções de data e horário

| Função | Responsabilidade |
|---|---|
| `formatarDataHora(millis)` | Converte o timestamp para texto no padrão brasileiro, como `15/07/2026 às 14:30`. |
| `extrairDataDoDatePicker(millisUtc)` | Lê ano, mês e dia do valor devolvido pelo `DatePicker`, que vem em milissegundos UTC (meia-noite UTC do dia escolhido). |
| `paraMillisUtcDoDatePicker(ano, mes, dia)` | Operação inversa, usada para abrir o seletor já posicionado na data salva. |
| `combinarDataHora(ano, mes, dia, hora, minuto)` | Monta o timestamp final no fuso do dispositivo. |

Separar as conversões em UTC e em fuso local evita que a data escolhida apareça deslocada em um dia em fusos como o de Brasília.

### 11. [`TarefaDAOTest.kt`](app/src/androidTest/java/arthurqzn/com/github/todolist/data/TarefaDAOTest.kt) — teste do acesso aos dados

O teste instrumentado cria um banco Room em memória antes de cada execução. Ele insere uma tarefa, coleta a primeira emissão de `listarTodas()` com `first()` e confirma quantidade, título e estado de conclusão. Um segundo teste confirma a ordenação: tarefas com prazo aparecem antes das avulsas e, entre elas, a de prazo mais próximo vem primeiro. Depois, fecha o banco.
...
3. O formulário identifica o ID `0` e entra no modo de cadastro, com data e horário opcionais.
4. Ao salvar, cria uma nova `Tarefa` (com `dataHora` ou `null`) e chama `TarefaViewModel.inserir()`.
...
```text
app/src/main/java/arthurqzn/com/github/todolist/
├── MainActivity.kt
├── data/
│   ├── Tarefa.kt
│   ├── TarefaDAO.kt
...
O projeto também possui, em `app/src/androidTest`, testes instrumentados do DAO (banco Room em memória) e de `DataHoraUtil` (formatação e conversões de data e hora).
...
![Tela inicial com a lista de tarefas](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/TelaInicial.png)
...
![Formulário preenchido para cadastro](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/CadastroNovaTarefa.png)
...
![Nova tarefa exibida na lista](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/TarefaNova_Cadastrada.png)
...
![Formulário no modo de edição](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/EditandoTarefa.png)
...
![Resultado da tarefa editada](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/TarefaEditada.png)
...
![Tarefa marcada como concluída](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/TarefaConcluida.png)
...
![Lista após a exclusão de duas tarefas](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/2TarefasRemovidas.png)
...
![Botão para navegar da lista ao formulário](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/NavegacaoLista.png)
...
![Formulário aberto pela navegação](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/NavegacaoFormulario.png)
...
![Ação de retorno do formulário](app/src/main/java/arthurqzn/com/github/todolist/docs/evidencias/FormularioParaLista.png)
